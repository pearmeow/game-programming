...
constexpr float TILE_DIMENSION = 50.0f,
    ...
    ACCELERATION_OF_GRAVITY = 981.0f, FIXED_TIMESTEP = 1.0f / 60.0f, END_GAME_THRESHOLD = 800.0f;
...
void initialise() {
    ...
    */
    ...
    gXochitl = new Entity({ORIGIN.x - 300.0f, ORIGIN.y - 200.0f},  // position
    ...
    gXochitl->setColliderDimensions({gXochitl->getScale().x / 3.0f, gXochitl->getScale().y / 3.0f});
    ...
    */
    ...
    for (int i = 0; i < NUMBER_OF_TILES; i++) {
        ...
        gTiles[i].setPosition({leftMostX + i * TILE_DIMENSION, ORIGIN.y + TILE_DIMENSION});
    }
...
void processInput() {
    ...
    if (GetLength(gXochitl->getMovement()) > 1.0f) gXochitl->normaliseMovement();
...
void update() {
    ...
    while (deltaTime >= FIXED_TIMESTEP) {
        gXochitl->update(FIXED_TIMESTEP, gTiles, NUMBER_OF_TILES, gBlocks, NUMBER_OF_BLOCKS);

        bool active = false;
        for (int i = 0; i < NUMBER_OF_BLOCKS; i++) {
            if (gBlocks[i].isActive()) active = true;
    ...
    if (gXochitl->getPosition().y > END_GAME_THRESHOLD) gAppStatus = TERMINATED;
}
