...
// Global Constants
constexpr int SCREEN_WIDTH = 1600 / 2, SCREEN_HEIGHT = 900 / 2, FPS = 120;
...
void initialise() {
    ...
    std::map<WitchState, std::vector<int>> animationAtlas = {{IDLE, {0, 1, 2, 3, 4, 5}},
                                                             {POWERING_UP, {0, 1, 2, 3, 4, 5, 6, 7}}};

    // Assets from @see https://9e0.itch.io/witches-pack
    witch = new Entity(ORIGIN,                                              // position
                       {250.0f, 250.0f},                                    // scale
                       {"assets/game/idle.png", "assets/game/attack.png"},  // texture file addresses
...
void processInput() {
    ...
    if (IsKeyPressed(KEY_P)) witch->togglePowerUp();
}
