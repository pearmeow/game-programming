...
Entity::Entity(Vector2 position, Vector2 scale, std::vector<const char*> textureFilepaths, TextureType textureType,
               Vector2 spriteSheetDimensions, std::map<WitchState, std::vector<int>> animationAtlas)
    : mPosition{position},
      mMovement{0.0f, 0.0f},
      mScale{scale},
      ...
      mWitchStatus{IDLE} {
    for (int i = 0; i < textureFilepaths.size(); i++) mTextures[(WitchState)i] = LoadTexture(textureFilepaths[i]);

    mCurrentTexture = mTextures[mWitchStatus];
}

Entity::~Entity() {
    for (int i = 0; i < mTextures.size(); i++) UnloadTexture(mTextures[(WitchState)i]);
};
...
void Entity::update(float deltaTime) {
    ...
    mPosition = {mPosition.x + mSpeed * mMovement.x * deltaTime, mPosition.y + mSpeed * mMovement.y * deltaTime};
...
void Entity::render() {
    ...
    switch (mTextureType) {
        case SINGLE:
            ...
            textureArea = {// top-left corner
                           ...
                           static_cast<float>(mCurrentTexture.width), static_cast<float>(mCurrentTexture.height)};
            break;
        case ATLAS:
            textureArea = getUVRectangle(&mCurrentTexture, mAnimationIndices[mCurrentFrameIndex],
    ...
    Rectangle destinationArea = {mPosition.x, mPosition.y, static_cast<float>(mScale.x),
                                 static_cast<float>(mScale.y)};
    printf("%f %f\n", mScale.x, mScale.y);

    // Origin inside the source texture (centre of the texture)
    Vector2 originOffset = {static_cast<float>(mScale.x) / 2.0f, static_cast<float>(mScale.y) / 2.0f};

    // Render the texture on screen
    DrawTexturePro(mCurrentTexture, textureArea, destinationArea, originOffset, mAngle, WHITE);
}

void Entity::togglePowerUp() {
